...
import {deleteAt, updateText, type DocHandle} from "@automerge/vanillajs"
import {
	asAutomergeURL,
	asDocumentURL,
	isValidAutomergeURL,
...
import type {OpenDocumentOptions} from ":/ui/dock/dock-api.ts"
import OpenWithContextMenu from ":/ui/dock/open-with.tsx"
...
export default function DocumentTree(props: {
	...
	const dockAPI = window.littlebook.dock
	const [_user, userHandle] = useUserContext()

	return (
		<div class="document-tree" data-depth={props.depth} role="group">
			<For each={props.urls}>
				{url => {
					...
					const pressed = () => dockAPI?.isPressed(url) ?? "false"
					const openDocument = (
						url: DocumentURL,
											...
											<Show when={isFolder(content())}>
												<ContextMenu.Item
													...
													onSelect={() => {
														...
														const contentHandle = entry()
															?.contentHandle as DocHandle<AreaDoc>

														if (contentHandle) {
															contentHandle.change(area => {
...
export function DocumentTreeItem(props: DocumentTreeItemProps) {
	...
	return (
		<Show when={isFolderish()} fallback={<DocumentTreeFile {...props} />}>
			<DocumentTreeFolder
				{...props}
...
export function DocumentTreeFile(props: DocumentTreeItemProps) {
	return (
		<Button
			class="document-tree-item document-tree-item--file"
			data-url={props.entry.url}
			onclick={() => props.openDocument(asDocumentURL(props.entry.url))}
			aria-pressed={props.pressed}>
			<div
				class="document-tree-item__indent"
				style={{
			...
			<span class="document-tree-item__icon">
				<IconPicker handle={props.entry.handle} fallback="📄" />
			</span>
			<span class="document-tree-item__name">
...
export function DocumentTreeFolder(
	props: DocumentTreeItemProps & {content: FolderShape},
) {
	...
	return (
		<>
			<Button
				...
				data-url={props.entry.url}
				class="popmenu__trigger popmenu__trigger--document-tree document-tree-item document-tree-item--folder"
				aria-pressed={props.pressed}
				onClick={() => props.openDocument(asDocumentURL(props.entry.url))}>
				<div
					class="document-tree-item__indent"
				...
				<button
					class="document-tree-item__expander"
					onClick={event => {
